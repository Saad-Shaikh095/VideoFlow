document.addEventListener('DOMContentLoaded', () => {
    const fetchBtn = document.getElementById('fetch-btn');
    const urlInput = document.getElementById('video-url');
    const resultContainer = document.getElementById('result-container');
    const errorMsg = document.getElementById('error-message');
    const loadingSpinner = fetchBtn.querySelector('.loading-spinner');
    const btnText = fetchBtn.querySelector('span');
    const sectionIcon = document.getElementById('section-icon');
    const tabBtns = document.querySelectorAll('.tab-btn');
    const tabIndicator = document.querySelector('.tab-indicator');

    let currentSection = 'youtube';

    const icons = {
        youtube: `<svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.872.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>`,
        instagram: `<svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/></svg>`
    };

    const placeholders = {
        youtube: 'Paste YouTube link here...',
        instagram: 'Paste Instagram Reel link here...'
    };

    // Tab Switching Logic
    tabBtns.forEach((btn, index) => {
        btn.addEventListener('click', () => {
            tabBtns.forEach(b => b.classList.remove('active'));
            btn.classList.add('active');

            const target = btn.dataset.target;
            currentSection = target;

            // Move indicator
            tabIndicator.style.transform = `translateX(${index * 100}%)`;

            // Update UI context
            sectionIcon.innerHTML = icons[target];
            urlInput.placeholder = placeholders[target];

            // Reset state
            hideError();
            resultContainer.classList.add('hidden');
            urlInput.value = '';
        });
    });

    fetchBtn.addEventListener('click', async () => {
        const url = urlInput.value.trim();
        if (!url) {
            showError(`Please paste a valid ${currentSection} link!`);
            return;
        }

        setLoading(true);
        hideError();
        resultContainer.classList.add('hidden');

        try {
            const response = await fetch('/fetch-info', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ url, type: currentSection }),
            });

            const data = await response.json();

            if (!response.ok) {
                throw new Error(data.detail || 'Failed to fetch video info');
            }

            renderResult(data);
        } catch (err) {
            showError(err.message);
        } finally {
            setLoading(false);
        }
    });

    function renderResult(data) {
        document.getElementById('video-title').textContent = data.title;
        document.getElementById('video-uploader').textContent = data.uploader || 'Unknown Uploader';
        document.getElementById('video-thumb').src = data.thumbnail;

        const durationText = data.duration ? formatDuration(data.duration) : 'N/A';
        document.getElementById('video-duration').textContent = durationText;

        const formatList = document.getElementById('format-list');
        formatList.innerHTML = '';

        data.formats.forEach(f => {
            const btn = document.createElement('a');
            btn.className = 'format-btn';

            // Use proxy download to force system-level download
            const proxyUrl = `/proxy-download?url=${encodeURIComponent(f.url)}&filename=${encodeURIComponent(data.title)}`;
            btn.href = proxyUrl;

            const size = f.filesize ? `(${(f.filesize / (1024 * 1024)).toFixed(1)} MB)` : '';
            btn.innerHTML = `
                ${f.resolution}
                <span>${f.ext.toUpperCase()} ${size}</span>
            `;

            formatList.appendChild(btn);
        });

        resultContainer.classList.remove('hidden');
        resultContainer.scrollIntoView({ behavior: 'smooth' });
    }

    function formatDuration(seconds) {
        const hrs = Math.floor(seconds / 3600);
        const mins = Math.floor((seconds % 3600) / 60);
        const secs = seconds % 60;
        return `${hrs > 0 ? hrs + ':' : ''}${mins.toString().padStart(2, '0')}:${secs.toString().padStart(2, '0')}`;
    }

    function setLoading(isLoading) {
        if (isLoading) {
            fetchBtn.disabled = true;
            loadingSpinner.classList.remove('hidden');
            btnText.classList.add('hidden');
        } else {
            fetchBtn.disabled = false;
            loadingSpinner.classList.add('hidden');
            btnText.classList.remove('hidden');
        }
    }

    function showError(msg) {
        errorMsg.textContent = msg;
        errorMsg.classList.remove('hidden');
    }

    function hideError() {
        errorMsg.classList.add('hidden');
    }

    // Support Enter key
    urlInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            fetchBtn.click();
        }
    });
});
